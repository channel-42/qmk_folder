...
#define RGBLIGHT_SPLIT_SET_CHANGE_MODEHSVS \
  rgblight_status.change_flags |=          \
      (RGBLIGHT_STATUS_CHANGE_MODE | RGBLIGHT_STATUS_CHANGE_HSVS)
...
#define RGBLIGHT_SPLIT_SET_CHANGE_TIMER_ENABLE \
  rgblight_status.change_flags |= RGBLIGHT_STATUS_CHANGE_TIMER
#define RGBLIGHT_SPLIT_ANIMATION_TICK \
  rgblight_status.change_flags |= RGBLIGHT_STATUS_ANIMATION_TICK
#else
#define RGBLIGHT_SPLIT_SET_CHANGE_MODE
#define RGBLIGHT_SPLIT_SET_CHANGE_HSVS
#define RGBLIGHT_SPLIT_SET_CHANGE_MODEHSVS
...
void sethsv(uint8_t hue, uint8_t sat, uint8_t val, LED_TYPE *led1) {
  sethsv_raw(hue, sat, val > RGBLIGHT_LIMIT_VAL ? RGBLIGHT_LIMIT_VAL : val,
             led1);
}
...
void rgblight_init(void) {
  /* if already initialized, don't do it again.
     ...
     This is a dirty, dirty hack until proper hooks can be added for keyboard
     startup. */
  ...
  if (!rgblight_config.mode) {
    dprintf(
        "rgblight_init rgblight_config.mode = 0. Write default values to "
        "EEPROM.\n");
    eeconfig_update_rgblight_default();
    rgblight_config.raw = eeconfig_read_rgblight();
  }
...
void rgblight_increase_hue_helper(bool write_to_eeprom) {
  ...
  rgblight_sethsv_eeprom_helper(hue, rgblight_config.sat, rgblight_config.val,
                                write_to_eeprom);
}
...
void rgblight_increase_sat_helper(bool write_to_eeprom) {
  ...
  rgblight_sethsv_eeprom_helper(rgblight_config.hue, sat, rgblight_config.val,
                                write_to_eeprom);
}
...
void rgblight_increase_val_helper(bool write_to_eeprom) {
  ...
  rgblight_sethsv_eeprom_helper(rgblight_config.hue, rgblight_config.sat, val,
                                write_to_eeprom);
}
...
void rgblight_increase_speed_helper(bool write_to_eeprom) {
  ...
  if (write_to_eeprom) {
    eeconfig_update_rgblight(
        rgblight_config.raw);  // EECONFIG needs to be increased to support this
  }
}
...
void rgblight_sethsv_eeprom_helper(uint8_t hue, uint8_t sat, uint8_t val,
  ...
  if (rgblight_config.enable) {
    ...
    if (rgblight_config.mode == RGBLIGHT_MODE_STATIC_LIGHT) {
      ...
      if (1 == 0) {  // dummy
...
#ifdef __AVR__
        // probably due to how pgm_read_word is defined for ARM, but the ARM
        // compiler really hates this line
        ...
        for (uint8_t i = 0; i < rgblight_ranges.effect_num_leds; i++) {
          uint8_t _hue =
              ((uint16_t)i * (uint16_t)range) / rgblight_ranges.effect_num_leds;
          if (direction) {
            _hue = hue + _hue;
          } else {
            _hue = hue - _hue;
          ...
          dprintf("rgblight rainbow set hsv: %d,%d,%d,%u\n", i, _hue, direction,
                  range);
          sethsv(_hue, sat, val,
                 (LED_TYPE *)&led[i + rgblight_ranges.effect_start_pos]);
        }
        rgblight_set();
      }
#endif
...
#ifdef RGBLIGHT_SPLIT
    if (rgblight_config.hue != hue || rgblight_config.sat != sat ||
    ...
    if (write_to_eeprom) {
      ...
      dprintf("rgblight set hsv [EEPROM]: %u,%u,%u\n", rgblight_config.hue,
              rgblight_config.sat, rgblight_config.val);
    } else {
...
void rgblight_setrgb(uint8_t r, uint8_t g, uint8_t b) {
  ...
  for (uint8_t i = rgblight_ranges.effect_start_pos;
       i < rgblight_ranges.effect_end_pos; i++) {
    led[i].r = r;
    led[i].g = g;
    led[i].b = b;
#ifdef RGBW
...
static uint8_t get_interval_time(const uint8_t *default_interval_address,
                                 uint8_t velocikey_min, uint8_t velocikey_max) {
  return
#ifdef VELOCIKEY_ENABLE
...
void rgblight_setrgb_range(uint8_t r, uint8_t g, uint8_t b, uint8_t start,
                           uint8_t end) {
  if (!rgblight_config.enable || start < 0 || start >= end ||
      end > RGBLED_NUM) {
    return;
  }
...
void rgblight_sethsv_range(uint8_t hue, uint8_t sat, uint8_t val, uint8_t start,
                           uint8_t end) {
  if (!rgblight_config.enable) {
    return;
  }
...
void rgblight_sethsv_slave(uint8_t hue, uint8_t sat, uint8_t val) {
  rgblight_sethsv_range(hue, sat, val, (uint8_t)RGBLED_NUM / 2,
                        (uint8_t)RGBLED_NUM);
}
...
static void rgblight_layers_write(void) {
  uint8_t i = 0;
  // For each layer
  for (const rgblight_segment_t *const *layer_ptr = rgblight_layers;
       i < RGBLIGHT_MAX_LAYERS; layer_ptr++, i++) {
    ...
    while (1) {
      ...
      LED_TYPE *const limit =
          &led[MIN(segment.index + segment.count, RGBLED_NUM)];
      for (LED_TYPE *led_ptr = &led[segment.index]; led_ptr < limit;
           led_ptr++) {
...
__attribute__((weak)) void rgblight_call_driver(LED_TYPE *start_led,
                                                uint8_t num_leds) {
...
void rgblight_update_sync(rgblight_syncinfo_t *syncinfo, bool write_to_eeprom) {
  ...
  if (syncinfo->status.change_flags & RGBLIGHT_STATUS_CHANGE_HSVS) {
    rgblight_sethsv_eeprom_helper(syncinfo->config.hue, syncinfo->config.sat,
                                  syncinfo->config.val, write_to_eeprom);
...
void rgblight_task(void) {
  if (rgblight_status.timer_enabled) {
    ...
    if (1 == 0) {  // dummy
      ...
      // breathing mode
      interval_time =
          get_interval_time(&RGBLED_BREATHING_INTERVALS[delta], 1, 100);
      ...
      // rainbow mood mode
      interval_time =
          get_interval_time(&RGBLED_RAINBOW_MOOD_INTERVALS[delta], 5, 100);
      ...
      // rainbow swirl mode
      interval_time =
          get_interval_time(&RGBLED_RAINBOW_SWIRL_INTERVALS[delta / 2], 1, 100);
      ...
      // snake mode
      interval_time =
          get_interval_time(&RGBLED_SNAKE_INTERVALS[delta / 2], 1, 200);
      ...
      // knight mode
      interval_time =
          get_interval_time(&RGBLED_KNIGHT_INTERVALS[delta], 5, 100);
      ...
      interval_time =
          get_interval_time(&RGBLED_TWINKLE_INTERVALS[delta % 3], 5, 50);
      effect_func = (effect_func_t)rgblight_effect_twinkle;
    }
#endif
#ifdef RGBLIGHT_EFFECT_PULSE
    else if (rgblight_status.base_mode == RGBLIGHT_MODE_PULSE) {
      interval_time = 100;
      effect_func = (effect_func_t)rgblight_effect_pulse;
    }
#endif
...
#else
  val = (exp(sin((anim->pos / 255.0) * M_PI)) -
         RGBLIGHT_EFFECT_BREATHE_CENTER / M_E) *
...
void rgblight_effect_rainbow_mood(animation_status_t *anim) {
  rgblight_sethsv_noeeprom_old(anim->current_hue, rgblight_config.sat,
                               rgblight_config.val);
  anim->current_hue++;
}
#endif
...
#ifdef RGBLIGHT_EFFECT_PULSE
void rgblight_effect_pulse(animation_status_t *anim) {
  uint8_t i;
  double val;
  for (i = 0; i < rgblight_ranges.effect_num_leds; i++) {
    val = RGBLIGHT_LIMIT_VAL * sin((0.05 * anim->pos) - (0.5 * (i%(RGBLED_NUM / 2))));
    if (val < 0) {
      val = 0.01;
    }
...
void rgblight_effect_rainbow_swirl(animation_status_t *anim) {
  ...
  for (i = 0; i < rgblight_ranges.effect_num_leds; i++) {
    hue = (RGBLIGHT_RAINBOW_SWIRL_RANGE / rgblight_ranges.effect_num_leds * i +
           anim->current_hue);
    sethsv(hue, rgblight_config.sat, rgblight_config.val,
           (LED_TYPE *)&led[i + rgblight_ranges.effect_start_pos]);
  }
  rgblight_set();
    ...
    for (j = 0; j < RGBLIGHT_EFFECT_SNAKE_LENGTH; j++) {
      ...
      if (i == k) {
        sethsv(
            ...
            (uint8_t)(rgblight_config.val * (RGBLIGHT_EFFECT_SNAKE_LENGTH - j) /
...
void rgblight_effect_knight(animation_status_t *anim) {
  ...
  for (i = rgblight_ranges.effect_start_pos; i < rgblight_ranges.effect_end_pos;
       i++) {
    led[i].r = 0;
    led[i].g = 0;
    led[i].b = 0;
  ...
  for (i = 0; i < RGBLIGHT_EFFECT_KNIGHT_LED_NUM; i++) {
    cur =
        (i + RGBLIGHT_EFFECT_KNIGHT_OFFSET) % rgblight_ranges.effect_num_leds +
        rgblight_ranges.effect_start_pos;

    if (i >= low_bound && i <= high_bound) {
      sethsv(rgblight_config.hue, rgblight_config.sat, rgblight_config.val,
             (LED_TYPE *)&led[cur]);
    } else {
      led[cur].r = 0;
      led[cur].g = 0;
...
void rgblight_effect_christmas(animation_status_t *anim) {
  ...
  // The pos is used in a cubic bezier formula to ease-in-out between red and
  // green, leaving the interpolated colors visible as short as possible.
  xa = CUBED((uint32_t)anim->pos);
  hue = ((uint32_t)hue_green) * xa /
        (xa + CUBED((uint32_t)(max_pos - anim->pos)));
  // Additionally, these interpolated colors get shown with a slightly darker
  // value, to make them less prominent than the main colors.
  val = 255 - (3 * (hue < hue_green / 2 ? hue : hue_green - hue) / 2);

  for (i = 0; i < rgblight_ranges.effect_num_leds; i++) {
    uint8_t local_hue =
        (i / RGBLIGHT_EFFECT_CHRISTMAS_STEP) % 2 ? hue : hue_green - hue;
    sethsv(local_hue, rgblight_config.sat, val,
           (LED_TYPE *)&led[i + rgblight_ranges.effect_start_pos]);
  }
  rgblight_set();
...
void rgblight_effect_twinkle(animation_status_t *anim) {
  ...
  for (uint8_t i = 0; i < rgblight_ranges.effect_num_leds; i++) {
    ...
    if (restart) {
      ...
      c->s = random_color ? (rand() % (rgblight_config.sat / 2)) +
